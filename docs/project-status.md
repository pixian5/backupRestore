...
更新时间：2026-09-24
当前开发版本：`1.6.6`（Cargo.toml×2 与 VERSION 已同步；完整当前基线见 [current-status-2026-09-16.md](current-status-2026-09-16.md)）
分支：`main`
本地开发基线：以当前 `HEAD` 为准

本文件是当前状态的唯一摘要。它把用户对下载、功能完整性和交付方式的明确要求记录为工程约束；不逐字保存情绪化表达，只保留会影响后续行为的事实和结论。

> 2026-09-16 起，涉及当前版本、未完成项和最新事实的结论以 [current-status-2026-09-16.md](current-status-2026-09-16.md) 为准；`current-progress-2026-09-13.md` 及更早文档降级为历史快照。本文件后面的时间线保留历史证据，不自动代表当前版本已通过。
