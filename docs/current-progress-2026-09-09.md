...
- **「返回 Windows」退出链路（2026-09-10/11 实机闭环）**：早期两轮失败定位——① v1 ShellExecuteW 静默失败（无退出码验证）；② PE 内 `/store` 场景 `{current}` 解析为 PE 自身条目（不在被编辑的 ESP BCD 里）→ bcdedit 失败；③ ExitWindowsEx(EWX_REBOOT) 在 PE 无权限静默返回（重启按钮靠 `wpeutil reboot` fallback 才有效）。**修复（v3→v7）**：部署期把 Windows 条目 GUID（`bcdedit /enum {current} /v` = `{d2264b2f-5431-11f1-9fa0-b55b60784edc}`）写入 ESP 根 `pe-exit-guid.txt`，PE 内读 GUID 直改 `default`（规避 {current}）；`pe_reboot()` 加 ExitWindowsEx→wpeutil 兜底；`write_pe_exit_log` 用 CreateFileW/WriteFile 直写卷路径 `\\?\Volume{GUID}\exit-pe.log`（std::fs 对 PE 挂载 FAT 卷写失败、盘符/卷路径在重启后失效）。**最终实机（2026-09-11 00:01，用户物理点击）**：点击「返回 Windows」→ cmd 显示 `The operation completed successfully. EXIT_CODE=0`（开发阶段保留 cmd+pause+`echo EXIT_CODE=%errorlevel%` 便于看真实输出）→ 按键后 `wpeutil reboot` → **真实重启直接进 Windows 无菜单**，BCD default 恢复 {current}（`_bcd-diag` 复核）。诊断弹窗显示完整链路：9 卷枚举（8×NTFS+1×FAT32 ESP `d08d796f`）→ 挂载 S: → BCD found → deploy GUID → bcdedit exit code 0 → 仅 `Q:\` 写失败 err=3（PE 无 Q: 盘符，预期）；X:\ 与 ESP 卷路径日志完整落盘，重启后经提升 PowerShell 读回验证。**日志读取教训**：cmd `mountvol S: /S` 未提升会静默失败 → 误判「NO_LOG」；须用提升 PowerShell。
...
- **结论**：路线 3 核心目标（PE 桌面中文显示 + 引导链 + **「返回 Windows」退出链实机闭环**）达成；0xc0000225 根因（BCD osloader 条目缺 systemroot/nx）与退出链三连坑（{current} 解析/ExitWindowsEx 权限/日志写入路径）均已修复并沉淀。遗留：PE 内卡片点击自动化（Parallels 注入限制）留用户手动（非产品缺陷）；开发阶段诊断弹窗（show_diag_dialog）+ cmd pause 在产品化时移除/收敛。
