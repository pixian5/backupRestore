...
- 默认分支：`main`
- 当前开发版本：以根目录 `VERSION` 为准；每完成一轮修改必须执行 `python3 ~/.codex/skills/pixian-dev-workflow/scripts/bump_version.py --root .`，同步两个 Cargo manifest 和 `Cargo.lock`。当前源码版本为 `0.9.3`；Windows 客体前台包需在本轮修改后重新构建，不能复用旧版本截图。
