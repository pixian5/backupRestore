...
| 备份 `.partial`、WIM 校验、SHA-256、metadata | `recover_windows` backup、`BackupMetadata`、原生输出按字节日志 | `v0.4.2` 代码已包含修复，实机待重跑 | DISM Capture 后检查 WIM、metadata 和 hash；旧 `v0.3.6` 任务曾被本地代码页日志解析缺陷阻断 |
...
| Rust GUI 三页面和二次确认 | `crates/backuprestore-cli/src/native_gui.rs`、`BackupRestore.exe` | ARM64 Windows 进程烟测已通过（窗口标题 `BackupRestore - Rust GUI`）；本机离线测试通过 | 真实按钮交互、管理员 UAC、长路径和日志刷新仍待解锁 VM 后验收 |
