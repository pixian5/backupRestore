...
当前版本基线：`1.3.5`。版本、失败根因和下一轮执行顺序见 [current-progress-2026-09-09.md](current-progress-2026-09-09.md)；下方历史任务 ID 保留为证据，不等同于最新版已回归。
...
| **PE 恢复环境一键安装** | `install-pe-secondary`（第 5 操作模式「PE 恢复」）、`run_cmd_to_file`（CreateProcessW+重定向）、`extract_bcd_guid`（GBK/UTF-16 自适应） | **实机已验证（v1.3.5 ARM64，2026-09-11 00:59）** | GUI 选 WIM + 目标卷 Q: → 复制 `C:\BackupRestorePE\BackupRestorePE.wim` 到 `Q:\sources\boot.wim`（SHA-256 与源一致）、`.stock` 保留旧 WIM、`Q:\boot\boot.sdi` 就位 → BCD 新增「Windows PE (BackupRestore)」osloader 条目（device/osdevice=ramdisk=[Q:]\sources\boot.wim,{ramdiskoptions}、winpe/detecthal/systemroot/nx），`default {current}` 未改动。弹窗「安装完成」。关键修复：bcdedit 不支持 `/create /application ramdisk` → 复用系统 `{ramdiskoptions}`；bcdedit 中文输出 GBK/UTF-16 → `extract_bcd_guid` 自适应解码并返回裸 GUID（双花括号 bug 已修）。菜单实启路径与 §4.10 的 PE Test 条目机制相同（ramdisk osloader），已有实机证据。 |
