...
| WIM 多索引读取与详细下拉显示 | `parse_wim_images`、`parse_dism_images`、`report_images_value` | **基础索引实机已验证（v0.7.4）；v1.0.8 ARM64 目标测试与同源双索引 WIM 生成已完成，GUI 按钮点检仍待完成** | `U:` 同一源卷两次捕获到 `V:\multi-index-same-source-v1.0.5.wim`，DISM 返回索引 1/2；v1.0.8 为 Win32 文本读取增加 WM_GETTEXT 回退，需在最新包点击“读取镜像”，确认下拉项显示两个索引及详细信息 |
