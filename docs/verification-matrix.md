...
| PE 恢复桌面（路线 3） | `--pe-desktop`、`winpeshl.ini`、WinPE-FontSupport-ZH-CN、BCD osloader（ramdisk/systemroot/nx） | **实机已验证（v1.3.3 ARM64，2026-09-10）** | 定制 boot.wim（含字体+产品 exe，SHA256 与源一致）部署 Q: + BCD（{179ca179} 条目：device/osdevice=ramdisk=[Q:]\sources\boot.wim,{ramdiskoptions}、winpe/detecthal/systemroot/nx）→ 真实重启 → PE 恢复桌面完整显示：标题、六卡「备份系统/还原系统/安装第二系统/命令提示符/返回 Windows/重启」全中文无乱码、底部版本栏+数字时钟走动（pe-desktop-zh.png）。0xc0000225 根因=osloader 条目缺 systemroot/nx，补后消失（§4.11）。「返回 Windows」退出逻辑代码就绪 + ESP BCD 路径实测可读；真实点击因 Parallels 无法注入 PE 输入留用户手动。验证后 BCD default 恢复 {current}，菜单模式（Windows 11 / Windows PE Test）重启自动进 Windows（win-back.png）。 |
